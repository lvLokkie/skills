...
| `plugins/skill-management/skills/skill-audit` | Overlaps with `writing-great-skills` pruning/no-op discipline | Local derivative idea, not a copied upstream skill | Keep as our own marketplace skill; use upstream as reference for vocabulary and invocation model. |
