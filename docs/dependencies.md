...
| `skill-management/skill-authoring` draft adapter only | `mattpocock/skills` | `in-progress/wizard` | reference / draft only | Wizard generation is useful for manual setup/migrations, but upstream marks it in-progress and it can touch secrets/env files. | Track as reviewed source material only; if adopted, keep under `plugins/in-progress` until a fresh promotion/security gate passes. | candidate |
