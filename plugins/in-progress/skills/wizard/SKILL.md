...
This is a draft, user-invoked skill imported under `plugins/in-progress` for evaluation. Do not list it in marketplace manifests or public install/run examples until a promotion audit decides it is safe and stable enough to publish.

Local guardrails:

- Treat generated wizard scripts as ephemeral unless the user explicitly wants a repeatable setup path committed to the repo.
- Do not run a generated wizard end-to-end yourself: it opens browsers, asks for human input, and may write `.env` values or GitHub secrets.
- Before committing a generated wizard, verify `bash -n`, optional `shellcheck`, no literal secrets, and exact CI secret/variable names.

## Attribution

Adapted from [`mattpocock/skills` `skills/in-progress/wizard`](https://github.com/mattpocock/skills/tree/main/skills/in-progress/wizard), MIT License, Copyright (c) 2026 Matt Pocock. See `THIRD_PARTY_NOTICES.md`.
