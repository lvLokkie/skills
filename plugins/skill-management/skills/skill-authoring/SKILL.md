...
2. **Check local value.** Keep a local skill only when it adds Ivan/Hermes routing, package indexes, dependency handling, verification, guardrails, or workflow behavior that an upstream dependency/reference does not already provide.
...
6. **Update package indexes.** Keep the destination `skills/README.md`, destination `plugin.json`, top-level README run examples/promoted table, and dependency docs aligned. When moving a skill, remove stale entries from the old category too.
7. **Bump versions.** Bump only the owning category plugin version when shipped behavior or command paths change. If moving between categories changes both old and new plugin surfaces, bump both affected plugin manifests.
8. **Run the publish gate.** Run `/skill-management:skill-audit` for the exact CRUD operation, then validation commands.
...
- [ ] Old category manifest and README no longer list it.
- [ ] New category manifest and README list it.
...
python -m json.tool plugins/<category>/.claude-plugin/plugin.json >/dev/null
```
...
Completion criterion: source files, README catalogs, manifests, dependency notes, versions, and validation agree; `/skill-management:skill-audit` has no unresolved packaging or security findings.
